...
class CatalogPage:
    ...
    def get_product_prices(self) -> list[float]:
        prices_test = self.product_cards.locator(".inventory_item_price").all_text_contents()
        return [float(p.replace("$", "")) for p in prices_test]
    ...
    def open_product_details(self, product_name: str):
        cart = self.product_cards.filter(has_text=product_name)

        name = cart.locator(".inventory_item_name").inner_text()
        price_text = cart.locator(".inventory_item_price").inner_text()
        price = float(price_text.replace("$", ""))

        cart.locator(".inventory_item_name").click()
